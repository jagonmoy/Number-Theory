...
int main() {
    ...
    while(t--) {
        ll n ;
        scanf("%lld",&n);
        MLL factors = factorialfactorization(n);
        VLL result ;
        ...
        if ( result.size() > 1) INC(i,0,result.size()-2) printf(" %lld (%lld) *",result[i].F,result[i].S);
         printf(" %lld (%lld)\n",result[result.size()-1].F,result[result.size()-1].S);    

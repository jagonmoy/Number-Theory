...
ll  check[1000005];
ll dgprimecount[1000005];

ll sumofdigit(ll n) {
    string s = numtostr(n);
    ll sum = 0 ;
    INC(i,0,s.size()-1) sum = sum + (ll)(s[i]-'0') ;
    return sum ;
}

void sieve(ll n) {
    memset(check,0,sizeof(check));
     memset(dgprimecount,0,sizeof(dgprimecount));
    check[1] = 1 ;
    ...
    INC(i,1,n) if ( check[i] == 0 && check[sumofdigit(i)] == 0) dgprimecount[i] = 1 ;
    INC(i,1,n) dgprimecount[i] = dgprimecount[i] + dgprimecount[i-1];   
    return ;
}
...
int main() {
    ...
    while(t--) {
        ll a,b ;
        scanf("%lld %lld",&a,&b);
        printf("%lld\n",dgprimecount[b]-dgprimecount[a-1]);
    }
